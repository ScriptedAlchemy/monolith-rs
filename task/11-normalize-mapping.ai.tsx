...
import { pythonRustMappingJson, pythonRustMappingGaps } from "./10-generate-mapping-table.ai.tsx";
...
function normalizeRustTargets(params: {
  ...
  notes: string;
  status: string;
}): { targets: NormalizedRustTarget[]; issues: string[]; naJustification?: string; suggestedRustTestLocation?: string } {
  ...
  const pushUnique = (t: NormalizedRustTarget) => {
    const key = t.fullPath ?? `${t.kind}:${t.raw ?? ""}:${t.crate ?? ""}:${t.path ?? ""}`;
    if (!seen.has(key)) {
      seen.add(key);
      targets.push(t);
    }
  ...
  if (normalizeStatus(status) === "N/A" || /^N\/A:/i.test(noteTrim)) {
    naJustification = noteTrim.startsWith("N/A:") || noteTrim.startsWith("N/A -") ? noteTrim : `N/A: ${noteTrim}`;
    if (naJustification === "N/A:") naJustification = "N/A: Unspecified justification.";
  ...
  for (const rt of rustTargets ?? []) {
    ...
    if (crate.length > 0) {
      ...
      if (trimmedPath.length > 0) {
        ...
        pushUnique({ kind: "crate_path", crate, path: trimmedPath, fullPath, raw: raw.length > 0 ? raw : undefined });
      } else {
        const fullPath = `monolith-rs/crates/${crate}`;
        pushUnique({ kind: "crate_root", crate, fullPath, raw: raw.length > 0 ? raw : undefined });
      }
      continue;
    }
    ...
    if (raw.length > 0) {
      ...
      if (m) {
        ...
        pushUnique({ kind: p.length > 0 ? "crate_path" : "crate_root", crate: c, path: p.length > 0 ? p : undefined, fullPath: cleaned, raw });
        continue;
      }
  ...
  if (isTest) {
    const anyTestsTarget = targets.some((t) => t.fullPath?.includes("/tests") || t.path?.startsWith("tests/") || t.path === "tests");
    if (!anyTestsTarget && naJustification == null) issues.push("python_test_should_map_to_rust_tests");
  }

  return { targets, issues, naJustification, suggestedRustTestLocation };
}

export const computeNormalizedMapping = action(async (actx): Promise<NormalizedMapping> => {
  const upstreamMappingJsonPath = "generated/parity/10-generate-mapping-table/python_rust_mapping_table.json";
  const upstreamGapsReportPath = "generated/parity/10-generate-mapping-table/python_rust_mapping_gaps.md";
  ...
  try {
    ...
    return {
      ...
      conventions: {
        ...
        rustTargetFormat: {
          canonical: "monolith-rs/crates/<crate>/(src|tests|examples|benches)/<path>",
          examples: ["monolith-rs/crates/monolith-core/src/lib.rs", "monolith-rs/crates/monolith-serving/tests/parity/agent_service/foo.rs"],
        },
        testMapping: {
          rule: "Python tests map to Rust tests (prefer crates/<crate>/tests/parity/...).",
          examples: ["monolith/tests/foo_test.py -> monolith-rs/crates/monolith-core/tests/parity/tests/foo_test.rs"],
        },
        fixtureMapping: {
          rule: "Golden fixtures used for parity live under a stable fixtures root and are shared by Python and Rust harnesses.",
          suggestedRoots: ["monolith-rs/fixtures/parity", "monolith-rs/testdata/parity"],
        },
        naPolicy: {
      ...
      counts: { pythonFiles: 0, recordsWithCrateTargets: 0, recordsWithEmptyTargets: 0, recordsWithNa: 0, recordsWithIssues: 0 },
      warnings: [`Missing ${upstreamMappingJsonPath}; run 10-generate-mapping-table first.`],
      records: [],
    };
  }
  ...
  try {
    ...
    warnings.push(`Failed to parse ${upstreamMappingJsonPath} as JSON; normalization will be empty.`);
    upstream = { records: [] };
  }

  const crateTomls = ((await actx.fs.glob(cratesGlob)) as string[]).slice().sort();
  ...
  if (crates.length === 0) warnings.push(`No crates discovered via ${cratesGlob}.`);

  const conventions: NormalizationConventions = {
    statusVocabulary: ["TODO", "IN_PROGRESS", "DONE", "N/A", "BLOCKED"],
    rustTargetFormat: {
      canonical: "monolith-rs/crates/<crate>/(src|tests|examples|benches)/<path>",
      examples: ["monolith-rs/crates/monolith-core/src/lib.rs", "monolith-rs/crates/monolith-serving/src/grpc/mod.rs", "monolith-rs/crates/monolith-core/tests/parity/core/hyperparams.rs"],
    },
    testMapping: {
      ...
      examples: ["monolith/core/tests/foo_test.py -> monolith-rs/crates/monolith-core/tests/parity/core/tests/foo_test.rs"],
    },
    fixtureMapping: {
    ...
    naPolicy: {
      ...
      examples: ["N/A: Documentation-only module; no runtime behavior used by production paths."],
    },
  };
  ...
  for (const r of upstream.records ?? []) {
    ...
    const { targets, issues, naJustification, suggestedRustTestLocation } = normalizeRustTargets({
    ...
    if (targets.some((t) => t.kind === "crate_path" || t.kind === "crate_root")) recordsWithCrateTargets += 1;
    if (naJustification != null || status === "N/A" || targets.some((t) => t.kind === "na")) recordsWithNa += 1;
...
export default (
  ...
  ><Asset id="mapping_conventions_doc" kind="doc" path="../generated/parity/11-normalize-mapping/mapping_conventions.md" /><Asset id="normalized_mapping_json" kind="json" path="../generated/parity/11-normalize-mapping/normalized_mapping.json" /><Action id="compute-normalized-mapping" export="computeNormalizedMapping" cache /><Agent id="write-normalized-mapping-json" produces={["normalized_mapping_json"]} external_needs={[{ alias: "pythonRustMappingJson", agent: "write-mapping-json" }, { alias: "pythonRustMappingGaps", agent: "write-mapping-gaps" }]}><Prompt><System>
          You maintain a parity planning pipeline. You produce strictly valid JSON and write files using apply_patch.
        </System><Context>{ctx.dependency(pythonRustMappingJson, { as: "Upstream mapping JSON (10-generate-mapping-table)", mode: "code" })}{ctx.dependency(pythonRustMappingGaps, { as: "Upstream mapping gaps report (10-generate-mapping-table)", mode: "quote" })}{ctx.actionResult("compute-normalized-mapping", { as: "Computed normalized mapping (canonical JSON)" })}</Context><Instructions>{`Write JSON to \`{{assets.normalized_mapping_json.path}}\` using apply_patch.
The JSON must be a single object and must exactly match the computed normalized mapping.`}</Instructions></Prompt></Agent><Agent id="write-mapping-conventions-doc" needs={["write-normalized-mapping-json"]} produces={["mapping_conventions_doc"]} external_needs={[{ alias: "pythonRustMappingJson", agent: "write-mapping-json" }]}><Prompt><System>
          You maintain a parity planning pipeline. You write deterministic operational docs and write files using apply_patch.
        </System><Context>{ctx.dependency(pythonRustMappingJson, { as: "Upstream mapping JSON (10-generate-mapping-table)", mode: "code" })}{ctx.actionResult("compute-normalized-mapping", { as: "Computed normalized mapping (JSON)" })}</Context><Instructions>{`Write a conventions document to \`{{assets.mapping_conventions_doc.path}}\` using apply_patch.
...
5) Include a short "Next normalization improvements" list, prioritized, without adding new requirements beyond what's already computed.`}</Instructions></Prompt></Agent></Program>
);
