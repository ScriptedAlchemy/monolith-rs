...
pub struct StockInstance {
    ...
    /// Current price features (normalized OHLCV)
    pub price_features: Vec<f32>,
    /// Technical indicator features
    pub indicator_features: Vec<f32>,

    /// Historical sequence for DIEN [lookback, features]
    pub historical_sequence: Vec<Vec<f32>>,
...
impl InstanceCreator {
    ...
    pub fn create_instances(
        ...
        for i in self.lookback_window..(n - self.forward_horizon) {
            ...
            let current_indicators = &indicators[i];
            ...
            let price_features = vec![
                current_bar.returns,
                (current_bar.high - current_bar.low) / current_price,
                (current_bar.close - current_bar.open) / current_price,
                (current_bar.volume / 1e7) as f32, // Normalize volume
            ];

            // Create historical sequence
            let mut historical_sequence = Vec::with_capacity(self.lookback_window);
            for j in (i - self.lookback_window)..i {
                let hist_bar = &bars[j];
                let hist_ind = &indicators[j];

                // Combine price and indicator features for each timestep
                let mut seq_features = vec![
                    hist_bar.returns,
                    (hist_bar.high - hist_bar.low) / hist_bar.close,
                    (hist_bar.close - hist_bar.open) / hist_bar.close,
                    (hist_bar.volume / 1e7) as f32,
                ];
                seq_features.extend(hist_ind.to_vec());
                historical_sequence.push(seq_features);
            }
            ...
            instances.push(StockInstance {
                ...
                price_features,
                indicator_features: current_indicators.to_vec(),
                historical_sequence,
                direction_label,
                magnitude_label,
                profitable_label,
