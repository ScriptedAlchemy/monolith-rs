...
mod tests {
    ...
    #[test]
    fn test_distributed_config_validate_allows_negative_barrier_timeout_for_ps_role() {
        let cfg = DistributedRunConfig {
            role: Role::Ps,
            barrier_timeout_ms: -1,
            ..DistributedRunConfig::default()
        ...
        cfg.validate().expect(
            "negative barrier timeout should be ignored for ps role because barrier waits are worker-only",
        );
    }
    ...
    #[tokio::test]
    async fn test_run_distributed_rejects_negative_barrier_timeout_for_worker_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Worker,
            barrier_timeout_ms: -1,
        ...
        let err = run_distributed(discovery, bad_cfg)
            .await
            .expect_err("run_distributed should reject worker config with negative barrier timeout");
        assert!(
            err.to_string().contains("barrier_timeout_ms > 0"),
            "unexpected runtime config validation error: {err}"
        );
    ...
    async fn test_run_worker_role_allows_parameter_sync_target_with_path_or_query_without_wrapper()
    {
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past parameter-sync path/query target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not parameter-sync path/query validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore parameter-sync path/query target validation errors: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_worker_role_allows_parameter_sync_target_with_userinfo_without_wrapper() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past parameter-sync userinfo target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not parameter-sync userinfo validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore parameter-sync userinfo target validation errors: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_worker_role_allows_empty_parameter_sync_target_entry_without_wrapper() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
            ...
            retry_backoff_ms: 1,
            heartbeat_interval: None,
            parameter_sync_targets: vec![String::new()],
            parameter_sync_interval: Duration::from_millis(0),
            parameter_sync_model_name: String::new(),
            parameter_sync_signature_name: String::new(),
            ..DistributedRunConfig::default()
        };
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past empty parameter-sync target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not empty parameter-sync target validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore empty parameter-sync target validation errors: {msg}"
        );
    }
    ...
    async fn test_run_worker_role_allows_duplicate_parameter_sync_targets_after_http_default_port_normalization_without_wrapper(
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past canonicalized duplicate parameter-sync target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not canonicalized duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore canonicalized duplicate parameter-sync target validation errors: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_worker_role_allows_duplicate_parameter_sync_targets_after_http_prefix_normalization_without_wrapper(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past http-prefix duplicate parameter-sync target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not http-prefix duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore http-prefix duplicate parameter-sync target validation errors: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_worker_role_allows_duplicate_parameter_sync_targets_after_https_default_port_normalization_without_wrapper(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past https default-port duplicate parameter-sync target validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not https default-port duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "worker role should ignore https default-port duplicate parameter-sync target validation errors: {msg}"
        );
    }
    ...
    #[tokio::test]
    async fn test_run_worker_role_allows_parameter_sync_names_with_internal_whitespace_without_wrapper(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            ...
            parameter_sync_model_name: "model name".to_string(),
            parameter_sync_signature_name: "signature name".to_string(),
            ..DistributedRunConfig::default()
        };
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past parameter-sync internal-whitespace name validation because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not parameter-sync name-shape validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_model_name")
                && !msg.contains("parameter_sync_signature_name"),
            "worker role should ignore parameter-sync name-shape validation errors: {msg}"
        );
    }
    ...
    #[tokio::test]
    async fn test_run_worker_role_allows_empty_parameter_sync_names_with_targets_without_wrapper() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
        ...
        let err = run_worker_role(discovery, "worker-0", cfg).await.expect_err(
            "worker role should proceed past empty parameter-sync name validation with targets because replication is ps-only",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "worker role should fail due to discovery timeout, not empty parameter-sync name validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_model_name")
                && !msg.contains("parameter_sync_signature_name"),
            "worker role should ignore empty parameter-sync name validation errors with targets: {msg}"
        );
    }
    ...
    async fn test_run_ps_role_rejects_empty_parameter_sync_target_entry_without_wrapper() {
        ...
        let err = run_ps_role(discovery, "ps-0", "ps".to_string(), bad_cfg).await.expect_err(
            "run_ps_role should reject empty parameter sync target entry",
        );
        assert!(
            err.to_string()
                .contains("non-empty parameter_sync_targets entries"),
            "unexpected ps-role validation error: {err}"
        );
    }
    ...
    #[tokio::test]
    async fn test_run_ps_role_rejects_duplicate_parameter_sync_target_entries_after_http_default_port_normalization_without_wrapper(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Ps,
        ...
        let err = run_ps_role(discovery, "ps-0", "ps".to_string(), bad_cfg).await.expect_err(
            "run_ps_role should reject normalized duplicate parameter-sync targets after default-port expansion",
        );
        assert!(
            err.to_string()
                .contains("requires unique parameter_sync_targets entries"),
    ...
    #[tokio::test]
    async fn test_run_ps_role_rejects_duplicate_parameter_sync_target_entries_after_trailing_slash_normalization_without_wrapper(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Ps,
        ...
        let err = run_ps_role(discovery, "ps-0", "ps".to_string(), bad_cfg).await.expect_err(
            "run_ps_role should reject normalized duplicate parameter-sync targets after trailing-slash normalization",
        );
        assert!(
            err.to_string()
                .contains("requires unique parameter_sync_targets entries"),
    ...
    #[tokio::test]
    async fn test_run_worker_role_rejects_negative_barrier_timeout_without_wrapper() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Worker,
            barrier_timeout_ms: -1,
        ...
        let err = run_worker_role(discovery, "worker-0", bad_cfg)
            .await
            .expect_err("run_worker_role should reject negative barrier timeout");
        assert!(
            err.to_string().contains("barrier_timeout_ms > 0"),
            "unexpected worker-role barrier-timeout validation error: {err}"
        );
    ...
    #[tokio::test]
    async fn test_run_ps_role_allows_negative_barrier_timeout_without_wrapper() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Ps,
            index: 0,
        ...
        assert!(
            !task.is_finished(),
            "run_ps_role should keep serving; negative worker-only barrier timeout must not fail ps role validation"
        );
        task.abort();
    }
    ...
    #[tokio::test]
    async fn test_run_distributed_rejects_empty_parameter_sync_target_entry_for_ps_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Ps,
            parameter_sync_targets: vec!["".to_string()],
        ...
        let err = run_distributed(discovery, bad_cfg).await.expect_err(
            "run_distributed should reject empty parameter sync target entry for ps role",
        );
        assert!(err
            .to_string()
            .contains("non-empty parameter_sync_targets entries"));
    ...
    #[tokio::test]
    async fn test_run_distributed_rejects_duplicate_parameter_sync_target_entries_after_http_default_port_normalization_for_ps_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Ps,
        ...
        let err = run_distributed(discovery, bad_cfg).await.expect_err(
            "run_distributed should reject normalized duplicate parameter-sync targets after default-port expansion for ps role",
        );
        assert!(
            err.to_string()
                .contains("requires unique parameter_sync_targets entries"),
    ...
    #[tokio::test]
    async fn test_run_distributed_rejects_duplicate_parameter_sync_target_entries_after_trailing_slash_normalization_for_ps_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let bad_cfg = DistributedRunConfig {
            role: Role::Ps,
        ...
        let err = run_distributed(discovery, bad_cfg).await.expect_err(
            "run_distributed should reject normalized duplicate parameter-sync targets after trailing-slash normalization for ps role",
        );
        assert!(
            err.to_string()
                .contains("requires unique parameter_sync_targets entries"),
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_parameter_sync_target_with_path_or_query_for_worker_role()
    {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past parameter-sync path/query target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not parameter-sync path/query validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "runtime should ignore parameter-sync path/query target validation errors for worker role: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_distributed_allows_parameter_sync_target_with_userinfo_for_worker_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past parameter-sync userinfo target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not parameter-sync userinfo validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "runtime should ignore parameter-sync userinfo target validation errors for worker role: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_distributed_allows_empty_parameter_sync_target_entry_for_worker_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past empty parameter-sync target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not empty parameter-sync target validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_duplicate_parameter_sync_targets_after_http_default_port_normalization_for_worker_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past canonicalized duplicate parameter-sync target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not canonicalized duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_duplicate_parameter_sync_targets_after_http_prefix_normalization_for_worker_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past http-prefix duplicate parameter-sync target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not http-prefix duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
            "runtime should ignore http-prefix duplicate parameter-sync target validation errors for worker role: {msg}"
        );
    }

    #[tokio::test]
    async fn test_run_distributed_allows_duplicate_parameter_sync_targets_after_https_default_port_normalization_for_worker_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past https default-port duplicate parameter-sync target validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not https default-port duplicate parameter-sync validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_targets"),
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_parameter_sync_names_with_internal_whitespace_for_worker_role(
    ) {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past parameter-sync internal-whitespace name validation for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not parameter-sync name-shape validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_model_name")
                && !msg.contains("parameter_sync_signature_name"),
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_empty_parameter_sync_names_with_targets_for_worker_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Worker,
            num_ps: 1,
        ...
        let err = run_distributed(discovery, cfg).await.expect_err(
            "run_distributed should proceed past empty parameter-sync name validation with targets for worker role",
        );
        let msg = err.to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "runtime should fail due to missing PS discovery, not empty parameter-sync name validation: {msg}"
        );
        assert!(
            !msg.contains("parameter_sync_model_name")
                && !msg.contains("parameter_sync_signature_name"),
            "runtime should ignore empty parameter-sync name validation errors with targets for worker role: {msg}"
        );
    }
    ...
    #[tokio::test]
    async fn test_run_distributed_allows_negative_barrier_timeout_for_ps_role() {
        let discovery = Arc::new(InMemoryDiscovery::new());
        let cfg = DistributedRunConfig {
            role: Role::Ps,
            index: 0,
        ...
        assert!(
            !task.is_finished(),
            "ps role should continue serving; negative worker-only barrier_timeout must not fail validation for ps role"
        );
        task.abort();
    }
    ...
    async fn test_run_distributed_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when non-index default-worker connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("default-worker connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out during discovery operation: connect worker-0 via worker after 20ms"),
    ...
    async fn test_run_distributed_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-worker connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("indexed custom-worker connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when ps connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("ps connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_fails_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-ps connect is blocked and cleanup disconnect fails");
        let msg = run_result
            .expect_err("indexed custom-ps connect timeout with cleanup disconnect failure should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_fails_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when custom non-index ps connect is blocked and cleanup disconnect fails");
        let msg = run_result
            .expect_err("custom non-index ps connect timeout with cleanup disconnect failure should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
                "Timed out during discovery operation: connect ps-0 via parameter_server_custom after 20ms"
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_fails_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed default-ps connect is blocked and cleanup disconnect fails");
        let msg = run_result
            .expect_err("indexed default-ps connect timeout with cleanup disconnect failure should surface as a role error")
            .to_string();
        assert!(
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_fails_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when non-index default-ps connect is blocked and cleanup disconnect fails");
        let msg = run_result
            .expect_err("default-ps connect timeout with cleanup disconnect failure should surface as a role error")
            .to_string();
        assert!(
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed default-ps connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("indexed default-ps connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out during discovery operation: connect ps-2 via ps after 20ms"),
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when non-index default-ps connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("default-ps connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out during discovery operation: connect ps-0 via ps after 20ms"),
    ...
    async fn test_run_distributed_ps_connect_timeout_preserves_error_when_disconnect_cleanup_times_out_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-ps connect and cleanup disconnect are both blocked");
        let msg = run_result
            .expect_err("indexed custom-ps connect timeout with cleanup disconnect timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
                "Timed out during discovery operation: connect ps-2 via parameter_server_custom after 20ms"
    ...
    async fn test_run_distributed_worker_discover_timeout_does_not_hang_and_cleans_up() {
        ...
        .await
        .expect("run_distributed should not hang when worker discovery operation blocks");
        let msg = run_result
            .expect_err("worker discover timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("last discovery error: Timed out during discovery operation: discover worker-0 for ps"),
    ...
    async fn test_run_distributed_worker_discover_timeout_includes_custom_service_type_context() {
        ...
        .await
        .expect("run_distributed should not hang when worker discover operation blocks");
        let msg = run_result
            .expect_err("worker discover timeout with custom service type should surface as a role error")
            .to_string();
        assert!(
            msg.contains(
                "last discovery error: Timed out during discovery operation: discover worker-0 for parameter_server_custom after 20ms"
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_times_out_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-service discover times out and cleanup steps block");
        let msg = run_result
            .expect_err("indexed custom-service discover timeout with cleanup timeouts should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_times_out_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed default discover times out and cleanup steps block");
        let msg = run_result
            .expect_err("indexed default discover timeout with cleanup timeouts should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "discover timeout should remain primary over cleanup timeouts: {msg}"
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_times_out_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when default-service non-index discover times out and cleanup steps block");
        let msg = run_result
            .expect_err("default non-index discover timeout with cleanup timeouts should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "discover timeout should remain primary over cleanup timeouts: {msg}"
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_fails_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed default-service discover times out and cleanup steps fail");
        let msg = run_result
            .expect_err("indexed default discover timeout with cleanup failures should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_fails_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when default-service non-index discover times out and cleanup steps fail");
        let msg = run_result
            .expect_err("default non-index discover timeout with cleanup failures should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "discover timeout should remain primary over cleanup failures: {msg}"
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_fails_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom discover times out and cleanup steps fail");
        let msg = run_result
            .expect_err("indexed custom discover timeout with cleanup failures should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
            "discover timeout should remain primary over cleanup failures: {msg}"
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_times_out_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when custom non-index discover times out and cleanup steps block");
        let msg = run_result
            .expect_err("custom non-index discover timeout with cleanup timeouts should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
    ...
    async fn test_run_distributed_worker_discover_timeout_preserves_error_when_cleanup_fails_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when custom non-index discover times out and cleanup steps fail");
        let msg = run_result
            .expect_err("custom non-index discover timeout with cleanup failures should surface as a role error")
            .to_string();
        assert!(
            msg.contains("Timed out waiting for PS discovery"),
    ...
    async fn test_run_distributed_connect_failure_does_not_hang_when_disconnect_blocks_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed worker connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("indexed worker connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_connect_failure_does_not_hang_when_disconnect_blocks_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when non-index worker connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("worker connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
    ...
    async fn test_run_distributed_connect_failure_does_not_hang_when_disconnect_blocks_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-worker connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("indexed custom-worker connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_connect_failure_does_not_hang_when_disconnect_blocks_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when custom non-index worker connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("custom worker connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_ps_connect_failure_does_not_hang_when_disconnect_blocks_with_custom_service_type(
        ...
        .await
        .expect("run_distributed should not hang when ps connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("custom ps connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_ps_connect_failure_does_not_hang_when_disconnect_blocks_with_custom_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed custom-ps connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("indexed custom-ps connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_ps_connect_failure_does_not_hang_when_disconnect_blocks_with_default_service_type_and_index(
        ...
        .await
        .expect("run_distributed should not hang when indexed ps connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("indexed default-ps connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
    ...
    async fn test_run_distributed_ps_connect_failure_does_not_hang_when_disconnect_blocks_with_default_service_type(
        ...
        .await
        .expect("run_distributed should not hang when non-index ps connect-failure cleanup disconnect blocks");
        let msg = run_result
            .expect_err("default-ps connect failure with disconnect cleanup timeout should surface as a role error")
            .to_string();
        assert!(
            msg.contains("forced connect failure"),
            "ps connect failure should remain primary when cleanup disconnect blocks: {msg}"
