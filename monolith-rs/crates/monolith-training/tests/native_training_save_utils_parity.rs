...
fn monolith_checkpoint_state_roundtrip_pbtxt() {
    ...
    let p = write_monolith_checkpoint_state(dir.path(), &st, true)
        .expect("write_monolith_checkpoint_state should succeed");
    assert!(p.exists());
...
fn monolith_checkpoint_state_remove_invalid_paths() {
    ...
    fs::write(&existing, "x").expect("writing existing checkpoint marker should succeed");
...
"#;
    fs::write(dir.path().join("monolith_checkpoint"), pbtxt)
        .expect("writing monolith_checkpoint pbtxt should succeed");
