...
### 537) Discovery Consul out-of-range register/deregister cleanup contracts
- Expanded Consul async cleanup coverage for out-of-range authority ports
  (`http://127.0.0.1:70000`) in
  `crates/monolith-training/src/discovery.rs`:
  - `register_async` regression verifies out-of-range config failures still
    compact dead watcher senders.
  - `deregister_async` regression verifies out-of-range config failures still:
...
### 541) Discovery ZooKeeper live-watcher preservation on register config failures
- Expanded ZooKeeper register failure watcher-lifecycle parity coverage in
  `crates/monolith-training/src/discovery.rs` for config-shape host failures:
  - `test_zk_async_register_malformed_ipv6_host_entry_keeps_live_watchers`
  - `test_zk_async_register_invalid_port_keeps_live_watchers`
- Both regressions verify that live watcher senders are preserved (not
  compacted) on register validation failure, complementing existing dead-watcher
  compaction regressions for the same error shapes.
- Result:
...
### 543) Discovery ZooKeeper invalid-port watch dead/live watcher symmetry
- Expanded ZooKeeper `watch_async` invalid-port lifecycle coverage in
  `crates/monolith-training/src/discovery.rs` with explicit dead/live watcher
  parity tests:
  - `test_zk_watch_async_invalid_port_compacts_dead_watch_sender`
  - `test_zk_watch_async_invalid_port_preserves_live_watch_sender`
- These complement the existing invalid-port no-state-creation check by
  validating watcher compaction and preservation behavior when watch subscribers
  already exist.
- Result:
...
### 545) Discovery watch dead/live watcher symmetry expansion across remaining config-shapes
- Expanded watch-level dead/live watcher symmetry coverage in
  `crates/monolith-training/src/discovery.rs` for remaining config-shape
  variants:
  - ZooKeeper malformed-host-entry (`[::1`) watch paths now cover:
    - dead watcher compaction,
    - live watcher preservation.
  - Consul out-of-range-port (`:70000`) watch paths now cover:
    - dead watcher compaction,
    - live watcher preservation.
  - Consul invalid-IPv6-suffix (`http://[::1]x:8500`) watch paths now cover:
    - dead watcher compaction,
    - live watcher preservation.
- Result:
  - watch lifecycle parity now has broader dead/live watcher symmetry across
...
### 549) Discovery Consul whitespace/empty-host watch dead/live watcher symmetry
- Expanded Consul `watch_async` whitespace-authority and empty-host lifecycle
  coverage in `crates/monolith-training/src/discovery.rs` with explicit
  dead/live watcher parity tests:
  - whitespace authority:
    - `test_consul_watch_async_whitespace_authority_compacts_dead_watch_sender`
    - `test_consul_watch_async_whitespace_authority_preserves_live_watch_sender`
  - empty host:
    - `test_consul_watch_async_empty_host_compacts_dead_watch_sender`
    - `test_consul_watch_async_empty_host_preserves_live_watch_sender`
- These complement existing reject-without-state checks by validating watcher
  compaction and preservation behavior when subscribers already exist.
- Result:
  - Consul whitespace-authority/empty-host watch paths now have full dead/live
    watcher symmetry coverage aligned with other config-error watch lanes.

### 550) Discovery Consul leading/trailing-whitespace watch dead/live watcher symmetry
- Expanded Consul `watch_async` leading/trailing-whitespace lifecycle coverage
  in `crates/monolith-training/src/discovery.rs` with explicit dead/live
  watcher parity tests:
  - `test_consul_watch_async_leading_trailing_whitespace_compacts_dead_watch_sender`
  - `test_consul_watch_async_leading_trailing_whitespace_preserves_live_watch_sender`
- These complement the existing reject-without-state check by validating watcher
  compaction and preservation behavior when subscribers already exist.
- Result:
...
### 552) Discovery ZooKeeper out-of-range watch symmetry and deregister cleanup
- Expanded ZooKeeper out-of-range host-port (`127.0.0.1:70000`) lifecycle
  coverage in `crates/monolith-training/src/discovery.rs` with:
  - watch dead/live watcher symmetry:
    - `test_zk_watch_async_out_of_range_port_compacts_dead_watch_sender`
    - `test_zk_watch_async_out_of_range_port_preserves_live_watch_sender`
  - deregister cleanup-notification contract:
    - `test_zk_async_deregister_out_of_range_port_still_notifies_and_returns_error`
- Result:
  - out-of-range host-port paths now explicitly verify watch sender
...
### 566) Discovery cross-feature compile-check after deregister symmetry expansion
- Performed a combined-feature compile/test smoke check for discovery tests
  after the latest Consul/ZooKeeper malformed-config deregister symmetry
  expansions:
  - `cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_address_fragment_compacts_dead_watchers -- --nocapture`
- Result:
...
### 568) Discovery Consul discover_async malformed-address cache-preservation expansion
- Expanded Consul malformed-address `discover_async` local-cache preservation
  regressions in `crates/monolith-training/src/discovery.rs`:
  - `test_consul_discover_async_out_of_range_port_preserves_local_cache`
  - `test_consul_discover_async_address_path_preserves_local_cache`
  - `test_consul_discover_async_invalid_ipv6_suffix_preserves_local_cache`
  - `test_consul_discover_async_leading_trailing_whitespace_preserves_local_cache`
- Result:
  - Consul discover cache-preservation coverage now includes additional parser
...
### 582) Discovery Consul host-port/root-slash watcher/cache symmetry tightening
- Expanded Consul watcher/cache lifecycle assertions in
  `crates/monolith-training/src/discovery.rs`:
  - Added live-watcher preservation assertions to:
    - `test_consul_async_deregister_host_port_without_scheme_uses_operation_context`
    - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_uses_operation_context`
  - Added host:port async register watcher-symmetry regressions:
    - `test_consul_async_register_host_port_without_scheme_compacts_dead_watchers`
    - `test_consul_async_register_host_port_without_scheme_keeps_live_watchers`
  - Added normalized-root-slash discover cache-preservation regression:
    - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_preserves_local_cache`
- Result:
  - Host-port/root-slash lanes now include explicit dead/live watcher symmetry
    and cache-retention guarantees across async discover/register/deregister
    operations.

### 583) Discovery Consul host-port deregister dead-watcher compaction parity
- Added `test_consul_async_deregister_host_port_without_scheme_compacts_dead_watchers`
  in `crates/monolith-training/src/discovery.rs`.
- Coverage validates that normalized host:port async deregister failure lanes
  compact dead watcher sender entries while preserving existing error-shape
  operation context (`deregister_entity` + `8501`).
- Result:
...
### 593) Discovery ZooKeeper empty-host watch/discover symmetry expansion
- Added additional empty-host regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_watch_async_empty_hosts_rejects_without_state_changes`
  - `test_zk_watch_async_empty_hosts_compacts_dead_watch_sender`
  - `test_zk_watch_async_empty_hosts_preserves_live_watch_sender`
  - `test_zk_discover_async_empty_hosts_preserves_local_cache`
- Coverage validates empty-host contracts across watch/discover paths:
  - watch rejects before creating state, compacts dead watchers, and preserves
    live watchers on config-error retries,
  - discover config errors preserve local cache entries.
- Result:
  - ZooKeeper empty-host parity now includes full watch/connect/register/
    deregister/discover lifecycle symmetry and cache-retention guarantees.

### 594) Discovery ZooKeeper empty-host-entry operation/lifecycle parity expansion
- Added empty-host-entry regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_empty_host_entry_is_config_error`
  - `test_zk_watch_async_empty_host_entry_rejects_without_state_changes`
  - `test_zk_watch_async_empty_host_entry_compacts_dead_watch_sender`
  - `test_zk_watch_async_empty_host_entry_preserves_live_watch_sender`
  - `test_zk_async_register_empty_host_entry_compacts_dead_watchers`
  - `test_zk_async_register_empty_host_entry_keeps_live_watchers`
  - `test_zk_discover_async_empty_host_entry_preserves_local_cache`
  - `test_zk_async_deregister_empty_host_entry_still_notifies_and_returns_error`
  - `test_zk_async_deregister_empty_host_entry_compacts_dead_watchers`
- Coverage validates deterministic `ConfigError` classification for
  `empty host entry` across connect/watch/register/discover/deregister plus
...
### 597) Discovery ZooKeeper valid multi-host async lifecycle parity expansion
- Added valid multi-host async regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_register_valid_multi_hosts_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_multi_hosts_failure_keeps_live_watchers`
  - `test_zk_async_deregister_valid_multi_hosts_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_multi_hosts_failure_compacts_dead_watchers`
  - `test_zk_discover_async_valid_multi_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_multi_hosts_connection_failure_preserves_local_cache`
- Coverage validates valid-but-unreachable multi-host failure-shape contracts
  and lifecycle symmetry across register/deregister/discover:
  - register/deregister return `ConnectionFailed` (not `ConfigError`),
  - deregister still emits `ServiceRemoved` and preserves local removal
    semantics,
  - dead watchers are compacted and live watchers preserved where expected,
  - discover failures preserve local cache entries.
- Result:
...
### 599) Discovery ZooKeeper valid multi-host watch/disconnect cleanup parity
- Added
  `test_zk_watch_async_valid_multi_hosts_disconnect_clears_poll_generation_with_live_receiver`
  in `crates/monolith-training/src/discovery.rs`.
- Coverage validates valid multi-host watch/disconnect lifecycle with a live
  receiver:
  - disconnect preserves live watcher sender entries,
  - disconnect clears watch poll-generation bookkeeping,
  - subsequent disconnect after receiver drop compacts dead watchers.
- Result:
  - ZooKeeper valid multi-host watch/disconnect parity now explicitly matches
    live/dead watcher cleanup guarantees used in other normalized address lanes.

### 600) Discovery ZooKeeper mixed-family host-list failure-shape parity
- Added mixed IPv4/IPv6 host-list regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_mixed_ipv4_ipv6_hosts_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_mixed_ipv4_ipv6_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_mixed_ipv4_ipv6_hosts_connection_failure_preserves_local_cache`
- Coverage validates syntactically valid mixed-family host lists are classified
  as `ConnectionFailed` (not `ConfigError`) when unreachable, and that
  discover-path failures preserve local cache entries.
- Result:
...
### 602) Discovery ZooKeeper mixed-family watch/disconnect cleanup parity
- Added
  `test_zk_watch_async_valid_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver`
  in `crates/monolith-training/src/discovery.rs`.
- Coverage validates mixed-family watch/disconnect lifecycle semantics with a
  live receiver:
  - disconnect preserves live watcher sender entries,
  - disconnect clears poll-generation bookkeeping,
  - subsequent disconnect after receiver drop compacts dead watchers.
- Result:
  - Mixed-family ZooKeeper parity now includes explicit watch/disconnect
    live/dead watcher cleanup guarantees in addition to connect/discover/
    register/deregister failure-shape coverage.

### 603) Discovery ZooKeeper mixed-family async retention parity tightening
- Added additional mixed IPv4/IPv6 async regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_register_valid_mixed_ipv4_ipv6_hosts_failure_does_not_cache_service`
  - `test_zk_async_deregister_valid_mixed_ipv4_ipv6_hosts_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_mixed_ipv4_ipv6_hosts_failure_keeps_live_watchers`
- Coverage tightens retention semantics for mixed-family valid-but-unreachable
  hosts:
  - register failures do not populate local cache,
  - deregister failures clear registered-path bookkeeping,
  - deregister preserves live watcher sender entries after notification.
- Result:
  - Mixed-family parity now aligns with single-endpoint and same-family
    multi-host retention guarantees across async lifecycle operations.

### 604) Discovery ZooKeeper host-only multi-host failure-shape parity
- Added host-only multi-host regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_multi_hosts_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_host_only_multi_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_multi_hosts_connection_failure_preserves_local_cache`
- Coverage validates syntactically valid host-only ZooKeeper host lists
  (`host,host`) are treated as valid-but-unreachable (`ConnectionFailed`), and
  discover async failures preserve local cache entries.
- Result:
  - ZooKeeper host-list parity now includes host-only multi-host inputs in the
    same failure-shape and cache-retention contracts as explicit port-based
    host lists.

### 605) Discovery ZooKeeper host-only register/deregister watcher symmetry expansion
- Added host-only multi-host async lifecycle regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_register_valid_host_only_multi_hosts_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_host_only_multi_hosts_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_host_only_multi_hosts_failure_does_not_cache_service`
  - `test_zk_async_deregister_valid_host_only_multi_hosts_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_host_only_multi_hosts_failure_compacts_dead_watchers`
- Coverage validates host-only valid-but-unreachable multi-host lists preserve
  async register/deregister lifecycle guarantees:
  - register failures compact dead watchers, preserve live watchers, and avoid
    cache pollution,
  - deregister failures still emit `ServiceRemoved`, preserve local removal
    semantics, and compact dead watchers.
- Result:
  - Host-only multi-host parity now extends beyond connect/discover to full
    register/deregister watcher and cache-retention symmetry contracts.

### 606) Discovery ZooKeeper host-only watch/disconnect cleanup parity
- Added
  `test_zk_watch_async_valid_host_only_multi_hosts_disconnect_clears_poll_generation_with_live_receiver`
  in `crates/monolith-training/src/discovery.rs`.
- Coverage validates host-only multi-host watch/disconnect lifecycle semantics
  with a live receiver:
  - disconnect preserves live watcher sender entries,
  - disconnect clears poll-generation bookkeeping,
  - subsequent disconnect after receiver drop compacts dead watchers.
- Result:
  - Host-only multi-host parity now includes explicit watch/disconnect
    live/dead watcher cleanup guarantees alongside connect/discover/
    register/deregister failure-shape contracts.

### 607) Discovery ZooKeeper host-only deregister retention parity tightening
- Added additional host-only multi-host deregister regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_deregister_valid_host_only_multi_hosts_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_multi_hosts_failure_keeps_live_watchers`
- Coverage tightens host-only deregister retention semantics on valid-but-
  unreachable endpoints:
  - registered-path bookkeeping is cleared even when backend delete fails,
  - live watcher sender entries remain after `ServiceRemoved` notification.
- Result:
  - Host-only multi-host parity now matches retention guarantees already
    enforced for single-endpoint, same-family, and mixed-family address lanes.

### 608) Discovery ZooKeeper host-only mixed-family failure-shape parity
- Added host-only mixed-family regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_mixed_ipv4_ipv6_hosts_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_host_only_mixed_ipv4_ipv6_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_mixed_ipv4_ipv6_hosts_connection_failure_preserves_local_cache`
- Coverage validates valid host-only mixed-family lists (`[::1],127.0.0.1`)
  are treated as valid-but-unreachable (`ConnectionFailed`), and discover-path
  async failures preserve local cache entries.
- Result:
...
### 610) Discovery ZooKeeper host-only mixed-family retention/cleanup tightening
- Added additional host-only mixed-family lifecycle regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_deregister_valid_host_only_mixed_ipv4_ipv6_hosts_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_mixed_ipv4_ipv6_hosts_failure_keeps_live_watchers`
  - `test_zk_watch_async_valid_host_only_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver`
- Coverage tightens retention and disconnect cleanup semantics:
  - deregister failures clear registered-path bookkeeping and preserve live
    watcher sender entries post-notification,
  - watch/disconnect preserves live watcher sender entries while clearing
    poll-generation state, then compacts dead watchers after receiver drop.
- Result:
  - Host-only mixed-family parity now includes full deregister retention and
    watch/disconnect cleanup guarantees aligned with other host-list lanes.

### 611) Discovery ZooKeeper host-only IPv6 multi-host failure-shape parity
- Added host-only IPv6 multi-host regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_ipv6_multi_hosts_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_host_only_ipv6_multi_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_ipv6_multi_hosts_connection_failure_preserves_local_cache`
- Coverage validates valid host-only IPv6 multi-host lists (`[::1],[::2]`) are
  classified as valid-but-unreachable (`ConnectionFailed`) rather than
  configuration errors, and discover-path async failures preserve local cache
  entries.
- Result:
  - ZooKeeper host-list parity now explicitly includes host-only IPv6
    multi-host failure-shape and cache-retention contracts.

### 612) Discovery ZooKeeper host-only IPv6 multi-host async lifecycle expansion
- Added host-only IPv6 multi-host async lifecycle regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_does_not_cache_service`
  - `test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_compacts_dead_watchers`
- Coverage validates host-only IPv6 multi-host valid-but-unreachable lists
  preserve async lifecycle guarantees:
  - register failures compact dead watchers, preserve live watchers, and avoid
    cache pollution,
  - deregister failures still remove local cache entries and emit
    `ServiceRemoved`, while compacting dead watchers.
- Result:
  - Host-only IPv6 multi-host parity now extends from connect/discover to
    async register/deregister watcher symmetry and cache-retention semantics.

### 613) Discovery ZooKeeper host-only IPv6 retention/cleanup tightening
- Added additional host-only IPv6 multi-host lifecycle regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_keeps_live_watchers`
  - `test_zk_watch_async_valid_host_only_ipv6_multi_hosts_disconnect_clears_poll_generation_with_live_receiver`
- Coverage tightens retention and disconnect cleanup semantics:
  - deregister failures clear registered-path bookkeeping and preserve live
    watcher sender entries after `ServiceRemoved` notification,
  - watch/disconnect preserves live watcher senders while clearing
    ...
    drop.
- Result:
  - Host-only IPv6 multi-host parity now includes full deregister retention and
    watch/disconnect cleanup guarantees aligned with other address lanes.

### 614) Discovery ZooKeeper IPv6 multi-host failure-shape parity
- Added IPv6 multi-host regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_ipv6_multi_hosts_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_ipv6_multi_hosts_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_ipv6_multi_hosts_connection_failure_preserves_local_cache`
- Coverage validates valid IPv6 multi-host lists with explicit ports
  (`[::1]:1,[::2]:2`) are treated as valid-but-unreachable
  (`ConnectionFailed`) rather than configuration errors, and discover async
  failures preserve local cache entries.
- Result:
...
### 617) Discovery ZooKeeper single-IPv6-host failure-shape parity
- Added single-IPv6-host regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_single_ipv6_host_returns_connection_failed_when_unreachable`
  - `test_zk_discover_async_valid_single_ipv6_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_single_ipv6_host_connection_failure_preserves_local_cache`
- Coverage validates a syntactically valid single IPv6 endpoint (`[::1]:1`) is
  treated as valid-but-unreachable (`ConnectionFailed`) and that discover async
  failures preserve local cache entries.
- Result:
...
### 619) Discovery ZooKeeper host-only single-IPv6 lifecycle parity
- Added host-only single-IPv6 regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_single_ipv6_host_returns_connection_failed_when_unreachable`
  - `test_zk_watch_async_valid_host_only_single_ipv6_host_disconnect_clears_poll_generation_with_live_receiver`
  - `test_zk_async_register_valid_host_only_single_ipv6_host_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_host_only_single_ipv6_host_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_host_only_single_ipv6_host_failure_does_not_cache_service`
  - `test_zk_discover_async_valid_host_only_single_ipv6_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_single_ipv6_host_connection_failure_preserves_local_cache`
  - `test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_compacts_dead_watchers`
  - `test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_keeps_live_watchers`
- Coverage validates host-only single-IPv6 inputs (`[::1]`) are treated as
  valid-but-unreachable (`ConnectionFailed`) and preserve full watcher/cache
  lifecycle semantics across watch/register/discover/deregister flows.
- Result:
  - Host-only single-IPv6 parity now matches all established cleanup and
    retention guarantees of the broader ZooKeeper valid-host lanes.

### 620) Discovery ZooKeeper host-only single-IPv4 lifecycle parity
- Added host-only single-IPv4 regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_single_ipv4_host_returns_connection_failed_when_unreachable`
  - `test_zk_watch_async_valid_host_only_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver`
  - `test_zk_async_register_valid_host_only_single_ipv4_host_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_host_only_single_ipv4_host_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_host_only_single_ipv4_host_failure_does_not_cache_service`
  - `test_zk_discover_async_valid_host_only_single_ipv4_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_single_ipv4_host_connection_failure_preserves_local_cache`
  - `test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_compacts_dead_watchers`
  - `test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_keeps_live_watchers`
- Coverage validates host-only single-IPv4 inputs (`127.0.0.1`) are treated as
  valid-but-unreachable (`ConnectionFailed`) and preserve full watcher/cache
  lifecycle semantics across watch/register/discover/deregister flows.
- Result:
  - Host-only single-IPv4 parity now matches established cleanup and retention
    guarantees of the broader ZooKeeper valid-host lanes.

### 621) Discovery ZooKeeper single-IPv4 lifecycle parity
- Added single-IPv4 regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_single_ipv4_host_returns_connection_failed_when_unreachable`
  - `test_zk_watch_async_valid_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver`
  - `test_zk_async_register_valid_single_ipv4_host_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_single_ipv4_host_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_single_ipv4_host_failure_does_not_cache_service`
  - `test_zk_discover_async_valid_single_ipv4_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_single_ipv4_host_connection_failure_preserves_local_cache`
  - `test_zk_async_deregister_valid_single_ipv4_host_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_single_ipv4_host_failure_compacts_dead_watchers`
  - `test_zk_async_deregister_valid_single_ipv4_host_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_single_ipv4_host_failure_keeps_live_watchers`
- Coverage validates explicit single-IPv4 host inputs (`127.0.0.1:1`) are
  treated as valid-but-unreachable (`ConnectionFailed`) and preserve full
  watcher/cache lifecycle semantics across watch/register/discover/deregister
  flows.
- Result:
  - Single-IPv4 parity now matches the lifecycle cleanup and retention
    guarantees already enforced across other valid-host address lanes.

### 622) Discovery ZooKeeper single-hostname lifecycle parity
- Added single-hostname regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_single_hostname_host_returns_connection_failed_when_unreachable`
  - `test_zk_watch_async_valid_single_hostname_host_disconnect_clears_poll_generation_with_live_receiver`
  - `test_zk_async_register_valid_single_hostname_host_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_single_hostname_host_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_single_hostname_host_failure_does_not_cache_service`
  - `test_zk_discover_async_valid_single_hostname_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_single_hostname_host_connection_failure_preserves_local_cache`
  - `test_zk_async_deregister_valid_single_hostname_host_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_single_hostname_host_failure_compacts_dead_watchers`
  - `test_zk_async_deregister_valid_single_hostname_host_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_single_hostname_host_failure_keeps_live_watchers`
- Coverage validates valid hostname endpoints with explicit ports
  (`localhost:1`) are treated as valid-but-unreachable (`ConnectionFailed`) and
  preserve watcher/cache lifecycle cleanup semantics across
  connect/watch/register/discover/deregister flows.
- Result:
  - Single-hostname parity now matches lifecycle cleanup and retention
    guarantees already enforced for IPv4/IPv6 valid-host lanes.

### 623) Discovery ZooKeeper host-only single-hostname lifecycle parity
- Added host-only single-hostname regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_host_only_single_hostname_host_returns_connection_failed_when_unreachable`
  - `test_zk_watch_async_valid_host_only_single_hostname_host_disconnect_clears_poll_generation_with_live_receiver`
  - `test_zk_async_register_valid_host_only_single_hostname_host_failure_compacts_dead_watchers`
  - `test_zk_async_register_valid_host_only_single_hostname_host_failure_keeps_live_watchers`
  - `test_zk_async_register_valid_host_only_single_hostname_host_failure_does_not_cache_service`
  - `test_zk_discover_async_valid_host_only_single_hostname_host_connection_failure_is_connection_failed`
  - `test_zk_discover_async_valid_host_only_single_hostname_host_connection_failure_preserves_local_cache`
  - `test_zk_async_deregister_valid_host_only_single_hostname_host_failure_still_removes_local_cache_and_notifies_watchers`
  - `test_zk_async_deregister_valid_host_only_single_hostname_host_failure_compacts_dead_watchers`
  - `test_zk_async_deregister_valid_host_only_single_hostname_host_failure_cleans_registered_path`
  - `test_zk_async_deregister_valid_host_only_single_hostname_host_failure_keeps_live_watchers`
- Coverage validates host-only hostname endpoints (`localhost`) are treated as
  valid-but-unreachable (`ConnectionFailed`) and retain deterministic
  watcher/cache cleanup semantics across
  connect/watch/register/discover/deregister flows.
- Result:
  - Host-only single-hostname parity now matches lifecycle cleanup and
    retention guarantees enforced across the existing IPv4/IPv6 and hostname
    address lanes.

### 624) Discovery Consul IPv6 host:port lifecycle parity
- Added Consul IPv6 host:port regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_with_port`
  - `test_consul_connect_ipv6_with_port_initializes_client_handle`
  - `test_consul_connect_ipv6_with_port_disconnect_and_reconnect`
  - `test_consul_watch_async_ipv6_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_async_register_ipv6_with_port_uses_operation_context`
  - `test_consul_async_register_ipv6_with_port_compacts_dead_watchers`
  - `test_consul_async_register_ipv6_with_port_keeps_live_watchers`
  - `test_consul_discover_async_ipv6_with_port_uses_operation_context`
  - `test_consul_discover_async_ipv6_with_port_preserves_local_cache`
  - `test_consul_async_deregister_ipv6_with_port_uses_operation_context`
  - `test_consul_async_deregister_ipv6_with_port_compacts_dead_watchers`
- Coverage validates bracketed IPv6 authority lanes (`[::1]:8501`) are treated
  as valid normalized endpoints:
  - normalization prepends `http://`,
  - connect/disconnect lifecycle remains healthy,
  - discover/register/deregister failure shape remains `Internal` with
    operation context,
  - watcher/poll-generation and cache cleanup semantics remain deterministic.
- Result:
...
### 628) Discovery Consul case-insensitive IPv6 register/watch symmetry completion
- Added targeted symmetry regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_consul_watch_async_case_insensitive_scheme_ipv6_seeds_poll_generation_entry`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_ipv6_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_ipv6_keeps_live_watchers`
- Coverage now mirrors existing case-insensitive/root-slash and host:port
  lifecycle expectations for IPv6:
  - watch poll-generation seeding + receiver-drop cleanup is explicit,
  - register failure paths now assert both dead-watcher compaction and
    live-watcher preservation.
- Result:
  - Case-insensitive IPv6 root-slash lane now has complete
    watch/register lifecycle symmetry with deterministic watcher cleanup
    guarantees.

### 629) Discovery Consul explicit-HTTPS lifecycle parity
- Added explicit-HTTPS regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_explicit_https_scheme`
  - `test_consul_connect_https_scheme_initializes_client_handle`
  - `test_consul_connect_https_scheme_disconnect_and_reconnect`
  - `test_consul_watch_async_https_scheme_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_async_register_https_scheme_uses_operation_context`
  - `test_consul_async_register_https_scheme_compacts_dead_watchers`
  - `test_consul_async_register_https_scheme_keeps_live_watchers`
  - `test_consul_discover_async_https_scheme_uses_operation_context`
  - `test_consul_discover_async_https_scheme_preserves_local_cache`
  - `test_consul_async_deregister_https_scheme_uses_operation_context`
  - `test_consul_async_deregister_https_scheme_compacts_dead_watchers`
- Coverage validates valid `https://` endpoints are accepted by normalization
  and preserve expected runtime failure shape/cleanup behavior:
  - connect/disconnect lifecycle remains healthy,
  - discover/register/deregister keep explicit `Internal` operation context,
  - watch disconnect continues deterministic poll-generation cleanup and watcher
    retention semantics.
- Result:
  - Consul lifecycle parity now explicitly includes `https://` authority lanes
    across connect/watch/register/discover/deregister.

### 630) Discovery Consul explicit-HTTPS IPv6 lifecycle parity
- Added explicit-HTTPS IPv6 regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_ipv6`
  - `test_consul_connect_https_ipv6_with_port_initializes_client_handle`
  - `test_consul_connect_https_ipv6_with_port_disconnect_and_reconnect`
  - `test_consul_watch_async_https_ipv6_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_async_register_https_ipv6_with_port_uses_operation_context`
  - `test_consul_async_register_https_ipv6_with_port_compacts_dead_watchers`
  - `test_consul_async_register_https_ipv6_with_port_keeps_live_watchers`
  - `test_consul_discover_async_https_ipv6_with_port_uses_operation_context`
  - `test_consul_discover_async_https_ipv6_with_port_preserves_local_cache`
  - `test_consul_async_deregister_https_ipv6_with_port_uses_operation_context`
  - `test_consul_async_deregister_https_ipv6_with_port_compacts_dead_watchers`
- Coverage validates `https://[::1]:8501` endpoint handling remains parity-safe:
  - normalization preserves explicit https IPv6 authorities,
  - connect/disconnect lifecycle remains healthy,
  - discover/register/deregister preserve explicit `Internal` operation context,
  - watch and deregister watcher/poll-generation cleanup semantics remain
    deterministic.
- Result:
  - Consul lifecycle parity now explicitly covers the HTTPS+IPv6 authority lane
    across connect/watch/register/discover/deregister flows.

### 631) Discovery Consul HTTPS watch poll-generation symmetry completion
- Added in `crates/monolith-training/src/discovery.rs`:
  - `test_consul_watch_async_https_scheme_seeds_poll_generation_entry`
  - `test_consul_watch_async_https_ipv6_with_port_seeds_poll_generation_entry`
- Coverage validates HTTPS lanes now mirror existing watch-lifecycle symmetry:
  - watch_async seeds poll-generation bookkeeping on subscribe,
  - dropped receiver paths deterministically clear poll-generation state,
  - dead watcher sender entries are compacted for both HTTPS IPv4 and HTTPS
    IPv6 authority forms.
- Result:
  - Consul HTTPS watcher lifecycle now has full seed+disconnect coverage parity
    across IPv4 and IPv6 lanes.

### 632) Discovery Consul case-insensitive HTTPS lifecycle parity
- Added case-insensitive HTTPS root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_disconnect_and_reconnect`
  - `test_consul_watch_async_case_insensitive_https_scheme_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_compacts_dead_watchers`
- Coverage validates `HtTpS://127.0.0.1:8501/` normalization and full lifecycle
  behavior:
  - scheme/root-slash canonicalization preserves valid HTTPS authority handling,
  - connect/disconnect lifecycle remains healthy,
  - watch poll-generation and sender cleanup remain deterministic,
  - discover/register/deregister preserve explicit `Internal` operation context
    and cache/watch cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive HTTPS
    authority handling across connect/watch/register/discover/deregister flows.

### 633) Discovery Consul case-insensitive HTTPS IPv6 lifecycle parity
- Added case-insensitive HTTPS IPv6 root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_compacts_dead_watchers`
- Coverage validates `HtTpS://[::1]:8501/` normalization and lifecycle handling:
  - scheme/root-slash canonicalization retains valid HTTPS IPv6 authority shape,
  - connect/disconnect behavior remains deterministic,
  - watch poll-generation + sender compaction semantics remain symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    with stable local-cache and watcher cleanup contracts.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive HTTPS IPv6
    authority handling across connect/watch/register/discover/deregister flows.

### 634) Discovery Consul hostname host:port lifecycle parity
- Added hostname host:port regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_with_port`
  - `test_consul_watch_async_hostname_port_without_scheme_seeds_poll_generation_entry`
  - `test_consul_watch_async_hostname_port_without_scheme_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_hostname_port_without_scheme_initializes_client_handle`
  - `test_consul_connect_hostname_port_without_scheme_disconnect_and_reconnect`
  - `test_consul_async_register_hostname_port_without_scheme_uses_operation_context`
  - `test_consul_async_register_hostname_port_without_scheme_compacts_dead_watchers`
  - `test_consul_async_register_hostname_port_without_scheme_keeps_live_watchers`
  - `test_consul_discover_async_hostname_port_without_scheme_uses_operation_context`
  - `test_consul_discover_async_hostname_port_without_scheme_preserves_local_cache`
  - `test_consul_async_deregister_hostname_port_without_scheme_uses_operation_context`
  - `test_consul_async_deregister_hostname_port_without_scheme_compacts_dead_watchers`
- Coverage validates `localhost:8501` authority handling remains parity-safe:
  - normalization prepends `http://` for hostname host:port authorities,
...
### 636) Discovery Consul case-insensitive IPv6 root-slash watch naming parity closure
- Added explicit IPv6 root-slash watch regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_ipv6_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_ipv6_disconnect_clears_poll_generation_with_live_receiver`
- Coverage validates the exact `HTTP://[::1]:8501/` lane now has watch-specific
  seed/disconnect tests named with full lane taxonomy:
  - watch subscribe seeds poll-generation bookkeeping,
  - disconnect clears poll-generation state while preserving live senders,
  - dropped receivers are compacted deterministically.
- Result:
  - Case-insensitive IPv6 root-slash lane now has explicit watch test-name
    symmetry aligned with connect/register/discover/deregister lane naming.

### 637) Discovery Consul case-insensitive HTTPS root-slash watch naming parity closure
- Added explicit HTTPS root-slash watch regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_disconnect_clears_poll_generation_with_live_receiver`
- Coverage validates exact `HtTpS://127.0.0.1:8501/` and
  `HtTpS://[::1]:8501/` lanes now have watch-specific seed/disconnect tests
  named with full lane taxonomy:
  - watch subscribe seeds poll-generation bookkeeping,
  - disconnect clears poll-generation state while preserving live senders,
  - dropped receivers are compacted deterministically.
- Result:
  - Case-insensitive HTTPS root-slash lanes now have explicit watch test-name
    symmetry aligned with connect/register/discover/deregister lane naming.

### 638) Discovery Consul hostname-without-port lifecycle parity
- Added hostname-without-port regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_without_port`
  - `test_consul_watch_async_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_hostname_without_port_initializes_client_handle`
  - `test_consul_connect_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_hostname_without_port_compacts_dead_watchers`
- Coverage validates `localhost` authority handling remains parity-safe:
  - normalization prepends `http://` for hostname-only authorities,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
...
### 640) Discovery Consul explicit-HTTPS hostname lifecycle parity
- Added explicit-HTTPS hostname regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname`
  - `test_consul_watch_async_https_hostname_with_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_https_hostname_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_https_hostname_with_port_initializes_client_handle`
  - `test_consul_connect_https_hostname_with_port_disconnect_and_reconnect`
  - `test_consul_async_register_https_hostname_with_port_uses_operation_context`
  - `test_consul_async_register_https_hostname_with_port_compacts_dead_watchers`
  - `test_consul_async_register_https_hostname_with_port_keeps_live_watchers`
  - `test_consul_discover_async_https_hostname_with_port_uses_operation_context`
  - `test_consul_discover_async_https_hostname_with_port_preserves_local_cache`
  - `test_consul_async_deregister_https_hostname_with_port_uses_operation_context`
  - `test_consul_async_deregister_https_hostname_with_port_compacts_dead_watchers`
- Coverage validates `https://localhost:8501` authority handling remains
  parity-safe:
  - normalization preserves explicit https hostname authorities,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes explicit-HTTPS hostname
    lanes across connect/watch/register/discover/deregister operations.

### 641) Discovery Consul explicit-HTTPS hostname-only lifecycle parity
- Added explicit-HTTPS hostname-only regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname_without_port`
  - `test_consul_watch_async_https_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_https_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_https_hostname_without_port_initializes_client_handle`
  - `test_consul_connect_https_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_https_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_https_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_https_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_https_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_https_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_https_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_https_hostname_without_port_compacts_dead_watchers`
- Coverage validates `https://localhost` authority handling remains parity-safe:
  - normalization preserves explicit https hostname-only authorities,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes explicit-HTTPS hostname-only
    lanes across connect/watch/register/discover/deregister operations.

### 642) Discovery Consul case-insensitive explicit-HTTPS hostname lifecycle parity
- Added case-insensitive explicit-HTTPS hostname regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_hostname_with_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_hostname_with_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_with_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_with_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_hostname_with_port_compacts_dead_watchers`
- Coverage validates `HtTpS://localhost:8501/` authority handling remains
  parity-safe:
  - normalization canonicalizes mixed-case https scheme for hostname authorities,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    explicit-HTTPS hostname-with-port lanes across
    connect/watch/register/discover/deregister operations.

### 643) Discovery Consul case-insensitive explicit-HTTPS hostname-only lifecycle parity
- Added case-insensitive explicit-HTTPS hostname-only regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_without_port`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_hostname_without_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_hostname_without_port_compacts_dead_watchers`
- Coverage validates `HtTpS://localhost/` authority handling remains
  parity-safe:
  - normalization canonicalizes mixed-case https scheme for hostname-only
    authorities and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
...
### 645) Discovery ZooKeeper connect disconnect/reconnect multi-host lifecycle parity
- Added ZooKeeper connect lifecycle regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_zk_connect_valid_multi_hosts_disconnect_and_reconnect`
  - `test_zk_connect_valid_mixed_ipv4_ipv6_hosts_disconnect_and_reconnect`
  - `test_zk_connect_valid_ipv6_multi_hosts_disconnect_and_reconnect`
  - `test_zk_connect_valid_host_only_multi_hosts_disconnect_and_reconnect`
  - `test_zk_connect_valid_host_only_mixed_ipv4_ipv6_hosts_disconnect_and_reconnect`
  - `test_zk_connect_valid_host_only_ipv6_multi_hosts_disconnect_and_reconnect`
- Coverage validates unreachable-but-valid multi-host lanes remain parity-safe:
  - initial connect attempts return explicit `ConnectionFailed` diagnostics,
  - disconnect remains successful and idempotent after failed connect attempts,
  - reconnect attempts preserve the same failure-shape contract,
  - failed connect/reconnect attempts do not leave stale client handles.
- Result:
  - ZooKeeper connect lifecycle parity now explicitly includes
    disconnect/reconnect determinism for multi-host host:port and host-only
    IPv4/IPv6/mixed-family authority permutations.

### 646) Discovery Consul case-insensitive explicit-HTTPS hostname (no root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS hostname regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_no_root_slash`
  - `test_consul_watch_async_case_insensitive_https_scheme_hostname_with_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_hostname_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_hostname_with_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_hostname_with_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_with_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_with_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_with_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_hostname_with_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_hostname_with_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_hostname_with_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_hostname_with_port_compacts_dead_watchers`
- Coverage validates `HtTpS://localhost:8501` authority handling remains
  parity-safe:
  - normalization canonicalizes mixed-case https scheme for hostname
    authorities without requiring a root slash suffix,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    explicit-HTTPS hostname-with-port lanes without root slash across
    connect/watch/register/discover/deregister operations.

### 647) Discovery Consul case-insensitive explicit-HTTPS hostname-only (no root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS hostname-only regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_without_port_no_root_slash`
  - `test_consul_watch_async_case_insensitive_https_scheme_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_hostname_without_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_hostname_without_port_compacts_dead_watchers`
- Coverage validates `HtTpS://localhost` authority handling remains parity-safe:
  - normalization canonicalizes mixed-case https scheme for hostname-only
    authorities without requiring a root slash suffix,
  - connect/disconnect client-handle lifecycle remains deterministic,
...
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    explicit-HTTPS hostname-only lanes without root slash across
    connect/watch/register/discover/deregister operations.

### 648) Discovery Consul case-insensitive explicit-HTTPS IP and IPv6 (no root-slash) lifecycle parity
- Added and/or tightened case-insensitive explicit-HTTPS no-root-slash
  regressions in `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_no_root_slash`
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_no_root_slash`
  - `test_consul_watch_async_case_insensitive_https_scheme_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_no_root_slash_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_no_root_slash_disconnect_and_reconnect`
  - `test_consul_connect_case_insensitive_https_scheme_ipv6_no_root_slash_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_ipv6_no_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_no_root_slash_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_no_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_no_root_slash_keeps_live_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_no_root_slash_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_no_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_no_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_no_root_slash_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_no_root_slash_preserves_local_cache`
  - `test_consul_discover_async_case_insensitive_https_scheme_ipv6_no_root_slash_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_ipv6_no_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_no_root_slash_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_no_root_slash_compacts_dead_watchers`
  - `test_consul_async_deregister_case_insensitive_https_scheme_ipv6_no_root_slash_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_ipv6_no_root_slash_compacts_dead_watchers`
- Coverage validates `HtTpS://127.0.0.1:8501`, `HtTpS://[::1]:8501`, and
  equivalent watch lanes remain parity-safe without requiring root slash:
  - normalization canonicalizes mixed-case https scheme and accepts no-root
...
### 650) Discovery Consul case-insensitive explicit-HTTP hostname-only (no root-slash) lifecycle parity
- Added case-insensitive explicit-HTTP hostname-only regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname_without_port_no_root_slash`
  - `test_consul_watch_async_case_insensitive_scheme_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_scheme_hostname_without_port_succeeds`
  - `test_consul_connect_case_insensitive_scheme_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_scheme_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_scheme_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_scheme_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_scheme_hostname_without_port_compacts_dead_watchers`
- Coverage validates `HtTp://localhost` authority handling remains parity-safe
  without requiring root slash:
  - normalization canonicalizes mixed-case scheme for hostname-only authority,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
...
### 652) Discovery Consul case-insensitive explicit-HTTP hostname-with-port (root-slash) lifecycle parity
- Added case-insensitive explicit-HTTP hostname-with-port + root-slash
  regressions in `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_with_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_with_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_with_port_succeeds`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_with_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_with_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_with_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_with_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_with_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_with_port_compacts_dead_watchers`
- Coverage validates `HtTp://localhost:8500/` authority handling remains
  parity-safe with optional root slash:
  - normalization canonicalizes mixed-case scheme and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    explicit-HTTP hostname-with-port lanes with root slash across
    connect/watch/register/discover/deregister operations.

### 653) Discovery Consul case-insensitive explicit-HTTP hostname-only (root-slash) lifecycle parity
- Added case-insensitive explicit-HTTP hostname-only + root-slash regressions
  in `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname_without_port`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_without_port_succeeds`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_without_port_compacts_dead_watchers`
- Coverage validates `HtTp://localhost/` authority handling remains parity-safe
  with optional root slash:
  - normalization canonicalizes mixed-case scheme and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
...
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    explicit-HTTP hostname-only lanes with root slash across
    connect/watch/register/discover/deregister operations.

### 654) Discovery Consul no-scheme hostname-with-port (root-slash) lifecycle parity
- Added no-scheme hostname-with-port + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_with_port_and_root_slash`
  - `test_consul_watch_async_hostname_port_without_scheme_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_hostname_port_without_scheme_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_hostname_port_without_scheme_and_root_slash_initializes_client_handle`
  - `test_consul_connect_hostname_port_without_scheme_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_hostname_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_register_hostname_port_without_scheme_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_hostname_port_without_scheme_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_hostname_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_hostname_port_without_scheme_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_hostname_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_hostname_port_without_scheme_and_root_slash_compacts_dead_watchers`
- Coverage validates `localhost:8501/` scheme-inference handling remains
  parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    hostname-with-port root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 655) Discovery Consul no-scheme hostname-only (root-slash) lifecycle parity
- Added no-scheme hostname-only + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_without_port_and_root_slash`
  - `test_consul_watch_async_hostname_without_port_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_hostname_without_port_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_hostname_without_port_and_root_slash_initializes_client_handle`
  - `test_consul_connect_hostname_without_port_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_hostname_without_port_and_root_slash_uses_operation_context`
  - `test_consul_async_register_hostname_without_port_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_hostname_without_port_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_hostname_without_port_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_hostname_without_port_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_hostname_without_port_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_hostname_without_port_and_root_slash_compacts_dead_watchers`
- Coverage validates `localhost/` scheme-inference handling remains parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    hostname-only root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 656) Discovery Consul no-scheme host:port (root-slash) lifecycle parity
- Added no-scheme host:port + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_host_port_and_root_slash`
  - `test_consul_watch_async_host_port_without_scheme_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_host_port_without_scheme_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_host_port_without_scheme_and_root_slash_initializes_client_handle`
  - `test_consul_connect_host_port_without_scheme_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_host_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_register_host_port_without_scheme_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_host_port_without_scheme_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_host_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_host_port_without_scheme_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_host_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_host_port_without_scheme_and_root_slash_compacts_dead_watchers`
- Coverage validates `127.0.0.1:8501/` scheme-inference handling remains
  parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    host:port root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 657) Discovery Consul no-scheme IPv6 host-only (root-slash) lifecycle parity
- Added no-scheme IPv6 host-only + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_without_port_and_root_slash`
  - `test_consul_watch_async_ipv6_without_port_without_scheme_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_ipv6_without_port_without_scheme_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_ipv6_without_port_without_scheme_and_root_slash_initializes_client_handle`
  - `test_consul_connect_ipv6_without_port_without_scheme_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_ipv6_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_register_ipv6_without_port_without_scheme_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_ipv6_without_port_without_scheme_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_ipv6_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_ipv6_without_port_without_scheme_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_ipv6_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_ipv6_without_port_without_scheme_and_root_slash_compacts_dead_watchers`
- Coverage validates `[::1]/` scheme-inference handling remains parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    IPv6 host-only root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 658) Discovery Consul no-scheme IPv6 host:port (root-slash) lifecycle parity
- Added no-scheme IPv6 host:port + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_with_port_and_root_slash`
  - `test_consul_watch_async_ipv6_with_port_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_ipv6_with_port_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_ipv6_with_port_and_root_slash_initializes_client_handle`
  - `test_consul_connect_ipv6_with_port_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_ipv6_with_port_and_root_slash_uses_operation_context`
  - `test_consul_async_register_ipv6_with_port_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_ipv6_with_port_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_ipv6_with_port_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_ipv6_with_port_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_ipv6_with_port_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_ipv6_with_port_and_root_slash_compacts_dead_watchers`
- Coverage validates `[::1]:8501/` scheme-inference handling remains parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    IPv6 host:port root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 659) Discovery Consul no-scheme host-only IPv4 lifecycle parity
- Added no-scheme host-only IPv4 regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port`
  - `test_consul_watch_async_host_without_port_without_scheme_seeds_poll_generation_entry`
  - `test_consul_watch_async_host_without_port_without_scheme_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_host_without_port_without_scheme_initializes_client_handle`
  - `test_consul_connect_host_without_port_without_scheme_disconnect_and_reconnect`
  - `test_consul_async_register_host_without_port_without_scheme_uses_operation_context`
  - `test_consul_async_register_host_without_port_without_scheme_compacts_dead_watchers`
  - `test_consul_async_register_host_without_port_without_scheme_keeps_live_watchers`
  - `test_consul_discover_async_host_without_port_without_scheme_uses_operation_context`
  - `test_consul_discover_async_host_without_port_without_scheme_preserves_local_cache`
  - `test_consul_async_deregister_host_without_port_without_scheme_uses_operation_context`
  - `test_consul_async_deregister_host_without_port_without_scheme_compacts_dead_watchers`
- Coverage validates `127.0.0.1` scheme-inference handling remains parity-safe:
  - normalization prepends `http://`,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    host-only IPv4 lanes across
    connect/watch/register/discover/deregister operations.

### 660) Discovery Consul no-scheme host-only IPv4 (root-slash) lifecycle parity
- Added no-scheme host-only IPv4 + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port_and_root_slash`
  - `test_consul_watch_async_host_without_port_without_scheme_and_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_host_without_port_without_scheme_and_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_host_without_port_without_scheme_and_root_slash_initializes_client_handle`
  - `test_consul_connect_host_without_port_without_scheme_and_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_host_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_register_host_without_port_without_scheme_and_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_host_without_port_without_scheme_and_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_host_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_discover_async_host_without_port_without_scheme_and_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_host_without_port_without_scheme_and_root_slash_uses_operation_context`
  - `test_consul_async_deregister_host_without_port_without_scheme_and_root_slash_compacts_dead_watchers`
- Coverage validates `127.0.0.1/` scheme-inference handling remains parity-safe:
  - normalization prepends `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
...
- Result:
  - Consul lifecycle parity now explicitly includes no-scheme
    host-only IPv4 root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 661) Discovery Consul case-insensitive HTTP host-only IPv4 (no-root-slash) lifecycle parity
- Added case-insensitive explicit-HTTP host-only IPv4 + no-root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port_no_root_slash`
  - `test_consul_watch_async_case_insensitive_scheme_host_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_host_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_scheme_host_without_port_succeeds`
  - `test_consul_connect_case_insensitive_scheme_host_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_scheme_host_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_scheme_host_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_host_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_scheme_host_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_scheme_host_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_scheme_host_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_scheme_host_without_port_compacts_dead_watchers`
- Coverage validates `HtTp://127.0.0.1` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `http://`,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    host-only IPv4 no-root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 662) Discovery Consul case-insensitive HTTP host-only IPv4 (root-slash) lifecycle parity
- Added case-insensitive explicit-HTTP host-only IPv4 + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_host_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_scheme_and_root_slash_host_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_host_without_port_succeeds`
  - `test_consul_connect_case_insensitive_scheme_and_root_slash_host_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_host_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_scheme_and_root_slash_host_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_scheme_and_root_slash_host_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_scheme_and_root_slash_host_without_port_compacts_dead_watchers`
- Coverage validates `HtTp://127.0.0.1/` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `http://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
...
### 663) Discovery Consul case-insensitive HTTPS host-only IPv4 (no-root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS host-only IPv4 + no-root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port_no_root_slash`
  - `test_consul_watch_async_case_insensitive_https_scheme_host_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_host_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_host_without_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_host_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_host_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_host_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_host_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_host_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_host_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_host_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_host_without_port_compacts_dead_watchers`
- Coverage validates `HtTpS://127.0.0.1` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `https://`,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    HTTPS host-only IPv4 no-root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 664) Discovery Consul case-insensitive HTTPS host-only IPv4 (root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS host-only IPv4 + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_host_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_host_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_host_without_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_host_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_host_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_host_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_host_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_host_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_host_without_port_compacts_dead_watchers`
- Coverage validates `HtTpS://127.0.0.1/` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `https://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic,
  - watch poll-generation state and watcher sender compaction stay symmetric,
  - register/discover/deregister preserve explicit `Internal` operation context
    and stable local-cache cleanup semantics.
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    HTTPS host-only IPv4 root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 665) Discovery Consul case-insensitive HTTPS host-only IPv6 (no-root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS host-only IPv6 + no-root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port_no_root_slash`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_compacts_dead_watchers`
- Coverage validates `HtTpS://[::1]` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `https://`,
  - connect/disconnect client-handle lifecycle remains deterministic for
...
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    HTTPS host-only IPv6 no-root-slash lanes across
    connect/watch/register/discover/deregister operations.

### 666) Discovery Consul case-insensitive HTTPS host-only IPv6 (root-slash) lifecycle parity
- Added case-insensitive explicit-HTTPS host-only IPv6 + root-slash regressions in
  `crates/monolith-training/src/discovery.rs`:
  - `test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_seeds_poll_generation_entry`
  - `test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_disconnect_clears_poll_generation_with_live_receiver`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_succeeds`
  - `test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_disconnect_and_reconnect`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_uses_operation_context`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_compacts_dead_watchers`
  - `test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_keeps_live_watchers`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_uses_operation_context`
  - `test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_preserves_local_cache`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_uses_operation_context`
  - `test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_compacts_dead_watchers`
- Coverage validates `HtTpS://[::1]/` normalization + lifecycle behavior remains parity-safe:
  - case-insensitive scheme canonicalizes to lowercase `https://` and trims root slash,
  - connect/disconnect client-handle lifecycle remains deterministic for
    host-only IPv6 authorities,
...
- Result:
  - Consul lifecycle parity now explicitly includes case-insensitive
    HTTPS host-only IPv6 root-slash lanes across
    connect/watch/register/discover/deregister operations.
...
1155. `cargo test -p monolith-training --features "consul" discovery::tests::test_consul_watch_async_whitespace_authority_compacts_dead_watch_sender -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_watch_async_whitespace_authority_preserves_live_watch_sender -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_watch_async_empty_host_compacts_dead_watch_sender -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_watch_async_empty_host_preserves_live_watch_sender -- --nocapture` ✅ (validated Consul whitespace-authority/empty-host watch dead/live watcher symmetry regressions)
...
1220. `cargo test -p monolith-training --features "consul" discovery::tests::test_consul_async_deregister_host_port_without_scheme_uses_operation_context -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_async_deregister_case_insensitive_scheme_and_root_slash_uses_operation_context -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_async_register_host_port_without_scheme_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_async_register_host_port_without_scheme_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "consul" discovery::tests::test_consul_discover_async_case_insensitive_scheme_and_root_slash_preserves_local_cache -- --nocapture` ✅ (validated expanded host-port/root-slash watcher and cache symmetry regressions)
...
1242. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_empty_hosts_rejects_without_state_changes -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_empty_hosts_compacts_dead_watch_sender -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_empty_hosts_preserves_live_watch_sender -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_empty_hosts_preserves_local_cache -- --nocapture` ✅ (validated ZooKeeper empty-host watch/discover state, watcher-symmetry, and cache-preservation regressions)
...
1254. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_multi_hosts_disconnect_clears_poll_generation_with_live_receiver -- --nocapture` ✅ (validated ZooKeeper valid multi-host watch/disconnect live-receiver cleanup regression)
...
1260. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver -- --nocapture` ✅ (validated ZooKeeper mixed-family watch/disconnect live-receiver cleanup regression)
1261. `rg "test_zk_watch_async_valid_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver" crates/monolith-training/src/discovery.rs` ✅ (verified new ZooKeeper mixed-family watch/disconnect live-receiver cleanup regression is present)
1262. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_mixed_ipv4_ipv6_hosts_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_mixed_ipv4_ipv6_hosts_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_mixed_ipv4_ipv6_hosts_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper mixed-family async cache-retention, registered-path cleanup, and live-watcher preservation regressions)
1263. `rg "test_zk_(async_register_valid_mixed_ipv4_ipv6_hosts_failure_does_not_cache_service|async_deregister_valid_mixed_ipv4_ipv6_hosts_failure_(cleans_registered_path|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper mixed-family async retention regression tests are present)
1264. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_host_only_multi_hosts_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_multi_hosts_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_multi_hosts_connection_failure_preserves_local_cache -- --nocapture` ✅ (validated ZooKeeper host-only multi-host connect/discover failure-shape and cache-retention regressions)
1265. `rg "test_zk_(connect_valid_host_only_multi_hosts_returns_connection_failed_when_unreachable|discover_async_valid_host_only_multi_hosts_connection_failure_(is_connection_failed|preserves_local_cache))" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper host-only multi-host regression tests are present)
1266. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_multi_hosts_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_multi_hosts_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_multi_hosts_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_multi_hosts_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_multi_hosts_failure_compacts_dead_watchers -- --nocapture` ✅ (validated ZooKeeper host-only multi-host async register/deregister watcher-symmetry and cache-retention regressions)
1267. `rg "test_zk_(async_register_valid_host_only_multi_hosts_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|async_deregister_valid_host_only_multi_hosts_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper host-only multi-host async lifecycle regression tests are present)
1268. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_multi_hosts_disconnect_clears_poll_generation_with_live_receiver -- --nocapture` ✅ (validated ZooKeeper host-only multi-host watch/disconnect live-receiver cleanup regression)
1269. `rg "test_zk_watch_async_valid_host_only_multi_hosts_disconnect_clears_poll_generation_with_live_receiver" crates/monolith-training/src/discovery.rs` ✅ (verified new ZooKeeper host-only multi-host watch/disconnect live-receiver cleanup regression is present)
1270. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_multi_hosts_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_multi_hosts_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper host-only multi-host deregister registered-path cleanup and live-watcher retention regressions)
...
1276. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_mixed_ipv4_ipv6_hosts_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_mixed_ipv4_ipv6_hosts_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver -- --nocapture` ✅ (validated ZooKeeper host-only mixed-family deregister retention and watch/disconnect cleanup regressions)
1277. `rg "test_zk_(async_deregister_valid_host_only_mixed_ipv4_ipv6_hosts_failure_(cleans_registered_path|keeps_live_watchers)|watch_async_valid_host_only_mixed_ipv4_ipv6_hosts_disconnect_clears_poll_generation_with_live_receiver)" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper host-only mixed-family retention/cleanup regression tests are present)
1278. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_host_only_ipv6_multi_hosts_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_ipv6_multi_hosts_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_ipv6_multi_hosts_connection_failure_preserves_local_cache -- --nocapture` ✅ (validated ZooKeeper host-only IPv6 multi-host connect/discover failure-shape and cache-retention regressions)
1279. `rg "test_zk_(connect_valid_host_only_ipv6_multi_hosts_returns_connection_failed_when_unreachable|discover_async_valid_host_only_ipv6_multi_hosts_connection_failure_(is_connection_failed|preserves_local_cache))" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper host-only IPv6 multi-host regression tests are present)
1280. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_ipv6_multi_hosts_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_compacts_dead_watchers -- --nocapture` ✅ (validated ZooKeeper host-only IPv6 multi-host async register/deregister watcher-symmetry and cache-retention regressions)
1281. `rg "test_zk_(async_register_valid_host_only_ipv6_multi_hosts_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|async_deregister_valid_host_only_ipv6_multi_hosts_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified newly added ZooKeeper host-only IPv6 multi-host async lifecycle regression tests are present)
1282. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_ipv6_multi_hosts_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_ipv6_multi_hosts_disconnect_clears_poll_generation_with_live_receiver -- --nocapture` ✅ (validated ZooKeeper host-only IPv6 multi-host deregister retention and watch/disconnect cleanup regressions)
...
1294. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_host_only_single_ipv6_host_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_single_ipv6_host_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv6_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv6_host_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv6_host_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_ipv6_host_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_ipv6_host_connection_failure_preserves_local_cache -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv6_host_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper host-only single-IPv6 lifecycle regressions across connect/watch/register/discover/deregister)
1295. `rg "test_zk_(connect_valid_host_only_single_ipv6_host_returns_connection_failed_when_unreachable|watch_async_valid_host_only_single_ipv6_host_disconnect_clears_poll_generation_with_live_receiver|async_register_valid_host_only_single_ipv6_host_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|discover_async_valid_host_only_single_ipv6_host_connection_failure_(is_connection_failed|preserves_local_cache)|async_deregister_valid_host_only_single_ipv6_host_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers|cleans_registered_path|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified host-only single-IPv6 lifecycle regression tests are present)
1296. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_host_only_single_ipv4_host_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv4_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv4_host_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_ipv4_host_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_ipv4_host_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_ipv4_host_connection_failure_preserves_local_cache -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_ipv4_host_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper host-only single-IPv4 lifecycle regressions across connect/watch/register/discover/deregister)
1297. `rg "test_zk_(connect_valid_host_only_single_ipv4_host_returns_connection_failed_when_unreachable|watch_async_valid_host_only_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver|async_register_valid_host_only_single_ipv4_host_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|discover_async_valid_host_only_single_ipv4_host_connection_failure_(is_connection_failed|preserves_local_cache)|async_deregister_valid_host_only_single_ipv4_host_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers|cleans_registered_path|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified host-only single-IPv4 lifecycle regression tests are present)
1298. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_single_ipv4_host_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_ipv4_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_ipv4_host_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_ipv4_host_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_single_ipv4_host_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_single_ipv4_host_connection_failure_preserves_local_cache -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_ipv4_host_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_ipv4_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_ipv4_host_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_ipv4_host_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper single-IPv4 lifecycle regressions across connect/watch/register/discover/deregister)
1299. `rg "test_zk_(connect_valid_single_ipv4_host_returns_connection_failed_when_unreachable|watch_async_valid_single_ipv4_host_disconnect_clears_poll_generation_with_live_receiver|async_register_valid_single_ipv4_host_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|discover_async_valid_single_ipv4_host_connection_failure_(is_connection_failed|preserves_local_cache)|async_deregister_valid_single_ipv4_host_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers|cleans_registered_path|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified single-IPv4 lifecycle regression tests are present)
1300. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_single_hostname_host_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_single_hostname_host_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_hostname_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_hostname_host_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_single_hostname_host_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_single_hostname_host_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_single_hostname_host_connection_failure_preserves_local_cache -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_hostname_host_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_hostname_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_hostname_host_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_single_hostname_host_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper single-hostname lifecycle regressions across connect/watch/register/discover/deregister)
1301. `rg "test_zk_(connect_valid_single_hostname_host_returns_connection_failed_when_unreachable|watch_async_valid_single_hostname_host_disconnect_clears_poll_generation_with_live_receiver|async_register_valid_single_hostname_host_failure_(compacts_dead_watchers|keeps_live_watchers|does_not_cache_service)|discover_async_valid_single_hostname_host_connection_failure_(is_connection_failed|preserves_local_cache)|async_deregister_valid_single_hostname_host_failure_(still_removes_local_cache_and_notifies_watchers|compacts_dead_watchers|cleans_registered_path|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified single-hostname lifecycle regression tests are present)
1302. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_host_only_single_hostname_host_returns_connection_failed_when_unreachable -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_watch_async_valid_host_only_single_hostname_host_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_hostname_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_hostname_host_failure_keeps_live_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_register_valid_host_only_single_hostname_host_failure_does_not_cache_service -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_hostname_host_connection_failure_is_connection_failed -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_discover_async_valid_host_only_single_hostname_host_connection_failure_preserves_local_cache -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_hostname_host_failure_still_removes_local_cache_and_notifies_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_hostname_host_failure_compacts_dead_watchers -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_hostname_host_failure_cleans_registered_path -- --nocapture && cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_async_deregister_valid_host_only_single_hostname_host_failure_keeps_live_watchers -- --nocapture` ✅ (validated ZooKeeper host-only single-hostname lifecycle regressions across connect/watch/register/discover/deregister)
...
1312. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_ipv6_seeds_poll_generation_entry -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_and_root_slash_ipv6_compacts_dead_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_and_root_slash_ipv6_keeps_live_watchers -- --nocapture` ✅ (validated case-insensitive IPv6 root-slash watch/register symmetry regressions for poll-generation seeding and dead/live watcher handling)
1313. `rg "test_consul_(watch_async_case_insensitive_scheme_ipv6_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|async_register_case_insensitive_scheme_and_root_slash_ipv6_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive IPv6 root-slash watch/register symmetry regression tests are present)
1314. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_explicit_https_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_scheme_initializes_client_handle -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_scheme_disconnect_and_reconnect -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_scheme_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_scheme_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_scheme_compacts_dead_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_scheme_keeps_live_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_scheme_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_scheme_preserves_local_cache -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_scheme_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_scheme_compacts_dead_watchers -- --nocapture` ✅ (validated explicit-HTTPS Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1315. `rg "test_(normalize_consul_address_for_operation_accepts_explicit_https_scheme|consul_(connect_https_scheme_(initializes_client_handle|disconnect_and_reconnect)|watch_async_https_scheme_disconnect_clears_poll_generation_with_live_receiver|async_register_https_scheme_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_https_scheme_(uses_operation_context|preserves_local_cache)|async_deregister_https_scheme_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified explicit-HTTPS Consul lifecycle regression tests are present)
1316. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_ipv6_with_port_initializes_client_handle -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_ipv6_with_port_disconnect_and_reconnect -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_ipv6_with_port_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_ipv6_with_port_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_ipv6_with_port_compacts_dead_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_ipv6_with_port_keeps_live_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_ipv6_with_port_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_ipv6_with_port_preserves_local_cache -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_ipv6_with_port_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_ipv6_with_port_compacts_dead_watchers -- --nocapture` ✅ (validated explicit-HTTPS IPv6 Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1317. `rg "test_(normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_ipv6|consul_(connect_https_ipv6_with_port_(initializes_client_handle|disconnect_and_reconnect)|watch_async_https_ipv6_with_port_disconnect_clears_poll_generation_with_live_receiver|async_register_https_ipv6_with_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_https_ipv6_with_port_(uses_operation_context|preserves_local_cache)|async_deregister_https_ipv6_with_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified explicit-HTTPS IPv6 Consul lifecycle regression tests are present)
1318. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_scheme_seeds_poll_generation_entry -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_ipv6_with_port_seeds_poll_generation_entry -- --nocapture` ✅ (validated explicit-HTTPS watch poll-generation seeding + dropped-receiver cleanup regressions for IPv4 and IPv6 lanes)
1319. `rg "test_consul_watch_async_https(_ipv6_with_port)?_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)" crates/monolith-training/src/discovery.rs` ✅ (verified explicit-HTTPS watch seed/disconnect lifecycle regression tests are present)
1320. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_succeeds -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_disconnect_and_reconnect -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_seeds_poll_generation_entry -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_disconnect_clears_poll_generation_with_live_receiver -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_compacts_dead_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_keeps_live_watchers -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_preserves_local_cache -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_uses_operation_context -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_compacts_dead_watchers -- --nocapture` ✅ (validated case-insensitive HTTPS Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1321. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme|consul_(connect_case_insensitive_https_scheme_and_root_slash_(succeeds|disconnect_and_reconnect)|watch_async_case_insensitive_https_scheme_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|async_register_case_insensitive_https_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive HTTPS Consul lifecycle regression tests are present)
1322. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6 -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6 -- --nocapture` ✅ (validated case-insensitive HTTPS IPv6 Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
...
1328. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_and_root_slash_ipv6 -- --nocapture` ✅ (validated case-insensitive IPv6 root-slash watch seed/disconnect lifecycle regressions)
1329. `rg "test_consul_watch_async_case_insensitive_scheme_and_root_slash_ipv6_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive IPv6 root-slash watch lifecycle regression tests are present)
1330. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_and_root_slash -- --nocapture` ✅ (validated case-insensitive HTTPS root-slash watch seed/disconnect lifecycle regressions for IPv4 and IPv6 lanes)
...
1336. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_hostname_with_port -- --nocapture` ✅ (validated explicit-HTTPS hostname Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1337. `rg "test_(normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname|consul_(watch_async_https_hostname_with_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_https_hostname_with_port_(initializes_client_handle|disconnect_and_reconnect)|async_register_https_hostname_with_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_https_hostname_with_port_(uses_operation_context|preserves_local_cache)|async_deregister_https_hostname_with_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified explicit-HTTPS hostname Consul lifecycle regression tests are present)
1338. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_https_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_https_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_https_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_https_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_https_hostname_without_port -- --nocapture` ✅ (validated explicit-HTTPS hostname-only Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1339. `rg "test_(normalize_consul_address_for_operation_accepts_explicit_https_scheme_with_hostname_without_port|consul_(watch_async_https_hostname_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_https_hostname_without_port_(initializes_client_handle|disconnect_and_reconnect)|async_register_https_hostname_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_https_hostname_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_https_hostname_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified explicit-HTTPS hostname-only Consul lifecycle regression tests are present)
1340. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_hostname_with_port -- --nocapture` ✅ (validated case-insensitive explicit-HTTPS hostname-with-port Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
...
1346. `cargo test -p monolith-training --features "zookeeper" discovery::tests::test_zk_connect_valid_ -- --nocapture` ✅ (validated ZooKeeper valid-host connect lifecycle regressions, including new multi-host disconnect/reconnect permutations)
1347. `rg "test_zk_connect_valid_(multi_hosts|mixed_ipv4_ipv6_hosts|ipv6_multi_hosts|host_only_multi_hosts|host_only_mixed_ipv4_ipv6_hosts|host_only_ipv6_multi_hosts)_disconnect_and_reconnect" crates/monolith-training/src/discovery.rs` ✅ (verified ZooKeeper multi-host disconnect/reconnect regression tests are present)
1348. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_hostname_with_port -- --nocapture` ✅ (validated case-insensitive explicit-HTTPS hostname-with-port no-root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1349. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_no_root_slash|consul_(watch_async_case_insensitive_https_scheme_hostname_with_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_https_scheme_hostname_with_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_https_scheme_hostname_with_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_hostname_with_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_hostname_with_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive explicit-HTTPS hostname-with-port no-root-slash Consul lifecycle regression tests are present)
1350. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_hostname_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_hostname_without_port -- --nocapture` ✅ (validated case-insensitive explicit-HTTPS hostname-only no-root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
...
1360. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_with_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_with_port -- --nocapture` ✅ (validated case-insensitive explicit-HTTP hostname-with-port root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1361. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname|consul_(watch_async_case_insensitive_scheme_and_root_slash_hostname_with_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_scheme_and_root_slash_hostname_with_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_scheme_and_root_slash_hostname_with_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_scheme_and_root_slash_hostname_with_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_scheme_and_root_slash_hostname_with_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive explicit-HTTP hostname-with-port root-slash lifecycle regression tests are present)
1362. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_and_root_slash_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_scheme_and_root_slash_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_and_root_slash_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_scheme_and_root_slash_hostname_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_scheme_and_root_slash_hostname_without_port -- --nocapture` ✅ (validated case-insensitive explicit-HTTP hostname-only root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1363. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_hostname_without_port|consul_(watch_async_case_insensitive_scheme_and_root_slash_hostname_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_scheme_and_root_slash_hostname_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_scheme_and_root_slash_hostname_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_scheme_and_root_slash_hostname_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_scheme_and_root_slash_hostname_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive explicit-HTTP hostname-only root-slash lifecycle regression tests are present)
1364. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_hostname_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_hostname_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_hostname_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_hostname_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_hostname_port_without_scheme_and_root_slash -- --nocapture` ✅ (validated no-scheme hostname-with-port root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1365. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_hostname_with_port_and_root_slash|consul_(watch_async_hostname_port_without_scheme_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_hostname_port_without_scheme_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_hostname_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_hostname_port_without_scheme_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_hostname_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme hostname-with-port root-slash lifecycle regression tests are present)
1366. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_hostname_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_hostname_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_hostname_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_hostname_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_hostname_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_hostname_without_port_and_root_slash -- --nocapture` ✅ (validated no-scheme hostname-only root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1367. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_hostname_without_port_and_root_slash|consul_(watch_async_hostname_without_port_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_hostname_without_port_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_hostname_without_port_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_hostname_without_port_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_hostname_without_port_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme hostname-only root-slash lifecycle regression tests are present)
1368. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_host_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_host_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_host_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_host_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_host_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_host_port_without_scheme_and_root_slash -- --nocapture` ✅ (validated no-scheme host:port root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1369. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_host_port_and_root_slash|consul_(watch_async_host_port_without_scheme_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_host_port_without_scheme_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_host_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_host_port_without_scheme_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_host_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme host:port root-slash lifecycle regression tests are present)
1370. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_ipv6_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_ipv6_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_ipv6_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_ipv6_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_ipv6_without_port_without_scheme_and_root_slash -- --nocapture` ✅ (validated no-scheme IPv6 host-only root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1371. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_without_port_and_root_slash|consul_(watch_async_ipv6_without_port_without_scheme_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_ipv6_without_port_without_scheme_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_ipv6_without_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_ipv6_without_port_without_scheme_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_ipv6_without_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme IPv6 host-only root-slash lifecycle regression tests are present)
1372. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_ipv6_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_ipv6_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_ipv6_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_ipv6_with_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_ipv6_with_port_and_root_slash -- --nocapture` ✅ (validated no-scheme IPv6 host:port root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1373. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_ipv6_with_port_and_root_slash|consul_(watch_async_ipv6_with_port_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_ipv6_with_port_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_ipv6_with_port_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_ipv6_with_port_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_ipv6_with_port_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme IPv6 host:port root-slash lifecycle regression tests are present)
1374. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_host_without_port_without_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_host_without_port_without_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_host_without_port_without_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_host_without_port_without_scheme -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_host_without_port_without_scheme -- --nocapture` ✅ (validated no-scheme host-only IPv4 Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1375. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port|consul_(watch_async_host_without_port_without_scheme_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_host_without_port_without_scheme_(initializes_client_handle|disconnect_and_reconnect)|async_register_host_without_port_without_scheme_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_host_without_port_without_scheme_(uses_operation_context|preserves_local_cache)|async_deregister_host_without_port_without_scheme_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme host-only IPv4 lifecycle regression tests are present)
1376. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_host_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_host_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_host_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_host_without_port_without_scheme_and_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_host_without_port_without_scheme_and_root_slash -- --nocapture` ✅ (validated no-scheme host-only IPv4 root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1377. `rg "test_(normalize_consul_address_for_operation_adds_http_scheme_to_host_without_port_and_root_slash|consul_(watch_async_host_without_port_without_scheme_and_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_host_without_port_without_scheme_and_root_slash_(initializes_client_handle|disconnect_and_reconnect)|async_register_host_without_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_host_without_port_without_scheme_and_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_host_without_port_without_scheme_and_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified no-scheme host-only IPv4 root-slash lifecycle regression tests are present)
1378. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_scheme_host_without_port -- --nocapture` ✅ (validated case-insensitive host-only IPv4 no-root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1379. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port_no_root_slash|consul_(watch_async_case_insensitive_scheme_host_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_scheme_host_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_scheme_host_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_scheme_host_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_scheme_host_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive host-only IPv4 no-root-slash lifecycle regression tests are present)
1380. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_scheme_and_root_slash_host_without_port -- --nocapture` ✅ (validated case-insensitive host-only IPv4 root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1381. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_scheme_with_host_without_port|consul_(watch_async_case_insensitive_scheme_and_root_slash_host_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_scheme_and_root_slash_host_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_scheme_and_root_slash_host_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_scheme_and_root_slash_host_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_scheme_and_root_slash_host_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive host-only IPv4 root-slash lifecycle regression tests are present)
1382. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_host_without_port -- --nocapture` ✅ (validated case-insensitive HTTPS host-only IPv4 no-root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1383. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port_no_root_slash|consul_(watch_async_case_insensitive_https_scheme_host_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_https_scheme_host_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_https_scheme_host_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_host_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_host_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive HTTPS host-only IPv4 no-root-slash lifecycle regression tests are present)
1384. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_host_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_host_without_port -- --nocapture` ✅ (validated case-insensitive HTTPS host-only IPv4 root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1385. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_host_without_port|consul_(watch_async_case_insensitive_https_scheme_and_root_slash_host_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_https_scheme_and_root_slash_host_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_https_scheme_and_root_slash_host_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_and_root_slash_host_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_and_root_slash_host_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive HTTPS host-only IPv4 root-slash lifecycle regression tests are present)
1386. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_ipv6_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_ipv6_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_ipv6_without_port_no_root_slash -- --nocapture` ✅ (validated case-insensitive HTTPS host-only IPv6 no-root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1387. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port_no_root_slash|consul_(watch_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_ipv6_without_port_no_root_slash_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive HTTPS host-only IPv6 no-root-slash lifecycle regression tests are present)
1388. `ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_connect_case_insensitive_https_scheme_and_root_slash_ipv6_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_register_case_insensitive_https_scheme_and_root_slash_ipv6_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port -- --nocapture && ZK_AUTH="user:pass" cargo test -p monolith-training --features "consul zookeeper" discovery::tests::test_consul_async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_without_port -- --nocapture` ✅ (validated case-insensitive HTTPS host-only IPv6 root-slash Consul lifecycle regressions across normalization/connect/watch/register/discover/deregister)
1389. `rg "test_(normalize_consul_address_for_operation_accepts_case_insensitive_https_scheme_with_ipv6_without_port|consul_(watch_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_(seeds_poll_generation_entry|disconnect_clears_poll_generation_with_live_receiver)|connect_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_(succeeds|disconnect_and_reconnect)|async_register_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_(uses_operation_context|compacts_dead_watchers|keeps_live_watchers)|discover_async_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_(uses_operation_context|preserves_local_cache)|async_deregister_case_insensitive_https_scheme_and_root_slash_ipv6_without_port_(uses_operation_context|compacts_dead_watchers)))" crates/monolith-training/src/discovery.rs` ✅ (verified case-insensitive HTTPS host-only IPv6 root-slash lifecycle regression tests are present)
